...
class ProtobufConsumerEncryptionExample {
 private:
  std::shared_ptr<ISchemaRegistryClient> client_;
  std::unique_ptr<ProtobufDeserializer<example::Author>> deserializer_;
  rd_kafka_t* consumer_;
  rd_kafka_conf_t* conf_;

 public:
  ProtobufConsumerEncryptionExample(const std::string& brokers, const std::string& group_id,
    ...
    // Create deserializer configuration with encryption support
    ...
    deserializer_ = std::make_unique<ProtobufDeserializer<example::Author>>(
        client_, nullptr, deser_config);
  ...
  ~ProtobufConsumerEncryptionExample() {
    if (consumer_) {
  ...
  void consume_and_print(const std::vector<std::string>& topics) {
    ...
    std::cout << "Consumer started. Waiting for encrypted messages..." << std::endl;
    ...
    while (true) {
      ...
      try {
        // Deserialize the message (encryption will be handled automatically)
        std::vector<uint8_t> payload(static_cast<const uint8_t*>(msg->payload),
                                   static_cast<const uint8_t*>(msg->payload) + msg->len);
        
        auto author_ptr = deserializer_->deserialize(ser_ctx, payload);
        const example::Author& author = *author_ptr;
        ...
        auto status = google::protobuf::util::MessageToJsonString(author, &json_string, options);
        if (!status.ok()) {
          std::cerr << "Error converting to JSON: " << status << std::endl;
          continue;
        }
        ...
        std::cout << "  Decrypted Payload: " << json_string << std::endl;
        ...
        std::cerr << "Error deserializing/decrypting message: " << e.what() << std::endl;
      }
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "Starting Protobuf consumer with encryption support:" << std::endl;
  ...
  try {
    ProtobufConsumerEncryptionExample consumer(brokers, group_id, schema_registry_url);
