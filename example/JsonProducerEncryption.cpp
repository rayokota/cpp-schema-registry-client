...
class JsonProducerEncryptionExample {
 private:
 ...
 public:
  JsonProducerEncryptionExample(const std::string& brokers,
    ...
    // Define the JSON schema with encryption rules
    std::string schema_str = R"(
{
  ...
  "properties": {
    ...
    SchemaSelector schema_selector = SchemaSelector::useLatestVersion();
    SerializerConfig ser_config(false, schema_selector, true, false, rule_config);

    // Create JSON serializer
    serializer_ = std::make_unique<JsonSerializer>(client_, std::nullopt, nullptr, ser_config);
      ...
      std::string schema_str = R"(
{
  ...
  void produce_messages(const std::string& topic_name) {
    ...
    for (int i = 0; i < 5; i++) {
      try {
        // Create JSON object (PII field "name" will be encrypted automatically)
        ...
        // Serialize the JSON object (encryption will be applied automatically)
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "Starting JSON producer with encryption support:" << std::endl;
  ...
  try {
    JsonProducerEncryptionExample producer(brokers, schema_registry_url, 
