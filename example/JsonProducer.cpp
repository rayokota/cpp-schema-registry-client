...
class JsonProducerExample {
 ...
 public:
  JsonProducerExample(const std::string& brokers,
    ...
    std::string schema_str = R"(
{
  ...
  "properties": {
    ...
    serializer_ = std::make_unique<JsonSerializer>(client_, schema, nullptr, ser_config);
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "Starting JSON producer with configuration:" << std::endl;
  std::cout << "  Brokers: " << brokers << std::endl;
  std::cout << "  Topic: " << topic << std::endl;
  std::cout << "  Schema Registry URL: " << schema_registry_url << std::endl;

  try {
    JsonProducerExample producer(brokers, schema_registry_url);
