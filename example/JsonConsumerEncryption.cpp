...
class JsonConsumerEncryptionExample {
 private:
  std::shared_ptr<ISchemaRegistryClient> client_;
  std::unique_ptr<JsonDeserializer> deserializer_;
  rd_kafka_t* consumer_;
  rd_kafka_conf_t* conf_;

 public:
  JsonConsumerEncryptionExample(const std::string& brokers, const std::string& group_id,
    ...
    deserializer_ = std::make_unique<JsonDeserializer>(client_, nullptr, deser_config);
  ...
  void consume_and_print(const std::vector<std::string>& topics) {
    ...
    while (true) {
      ...
      try {
        ...
        nlohmann::json json_result = deserializer_->deserialize(ser_ctx, payload);
        ...
        std::cout << "  Decrypted Payload: " << json_result.dump(2) << std::endl;
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "Starting JSON consumer with encryption support:" << std::endl;
  ...
  try {
    JsonConsumerEncryptionExample consumer(brokers, group_id, schema_registry_url);
