...
public:
    SchemaId() = default;
    SchemaId(SerdeFormat serde_format, 
    ...
    std::optional<std::vector<int32_t>> getMessageIndexes() const { return message_indexes_; }
    
    // Setters
    void setId(int32_t id) { id_ = id; }
    void setGuid(const std::string& guid) { guid_ = guid; }
    void setMessageIndexes(const std::vector<int32_t>& indexes) { message_indexes_ = indexes; }
...
struct SerdeHeader {
    ...
    SerdeHeader() = default;
    SerdeHeader(const std::string& k, const std::optional<std::vector<uint8_t>>& v = std::nullopt)
        : key(k), value(v) {}
...
public:
    ...
    std::optional<std::vector<uint8_t>> getLastHeaderValue(const std::string& key) const;
...
public:
    ...
    // Accessors
    const SerdeValue& getContainingMessage() const { return containing_message_; }
...
private:
    ...
    std::unordered_map<std::string,std::unordered_set<std::string>> inline_tags_;
...
public:
    ...
    std::optional<std::unordered_set<std::string>> getInlineTags(const std::string& name) const;
...
public:
    ...
    std::optional<RegisteredSchema> getReaderSchema(const std::string& subject,
    ...
    std::vector<Migration> getMigrations(const std::string& subject,
                                       const Schema& source_info,
                                       const RegisteredSchema& target,
                                       std::optional<std::string> format = std::nullopt) const;
    
    std::vector<RegisteredSchema> getSchemasBetween(const std::string& subject,
...
private:
    ...
    void runAction(const RuleContext& ctx,
                  Mode rule_mode,
                  const Rule& rule,
    ...
    std::shared_ptr<RuleAction> getRuleAction(const RuleContext& ctx, 
                                             const std::string& action_name) const;
    
    bool hasRules(std::optional<RuleSet> rule_set, Phase phase, Mode mode) const;
};
...
public:
    ...
    // Copy/move constructors and assignment operators - deleted due to Serde containing non-copyable RuleRegistry
...
public:
    ...
    Schema getWriterSchema(const SchemaId& schema_id,
