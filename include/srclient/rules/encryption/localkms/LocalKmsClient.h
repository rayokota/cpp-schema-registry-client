 ...
 * 
 * This class implements a KMS client that doesn't rely on external KMS services.
 * Instead, it derives encryption keys from a local secret using HKDF and creates
 * AES-GCM AEAD primitives for encryption/decryption operations.
 * 
...
public:
     ...
     */
    static absl::StatusOr<std::vector<uint8_t>> GetKey(const std::string& secret);
     ...
     */
    static constexpr absl::string_view GetPrefix() {
        return "local-kms://";
    }
