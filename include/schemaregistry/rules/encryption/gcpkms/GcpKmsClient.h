...
#include "schemaregistry/rules/encryption/gcpkms/GcpKmsAead.h"
#include "tink/aead.h"
...
class GcpKmsClient : public crypto::tink::KmsClient {
  public:
    ...
    // Creates a new GcpKmsClient that is bound to the key specified in
    // `key_uri`, and that uses the specified credentials when communicating
    // with the KMS.
    //
    // Either argument can be empty.
    // If `key_uri` is empty, then the client is not bound to any particular
    // key. If `credential_path` is empty, then default credentials will be
    // used.
    ...
    // operation. Avoid registering a client more than once.
    //
