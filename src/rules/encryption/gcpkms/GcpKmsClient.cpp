...
absl::StatusOr<std::shared_ptr<grpc::ChannelCredentials>> GetCredentials(
    ...
    // Creating "empty" 'channel_creds', to convert 'creds' to
    // ChannelCredentials via CompositeChannelCredentials().
...
absl::StatusOr<std::string> GetKeyName(absl::string_view key_uri) {
    if (!absl::StartsWithIgnoreCase(key_uri, kKeyUriPrefix)) {
        return absl::Status(
            absl::StatusCode::kInvalidArgument,
            absl::StrCat("The key URI ", key_uri, " does not start with ",
                         kKeyUriPrefix));
    }
...
absl::StatusOr<std::unique_ptr<Aead>> GcpKmsClient::GetAead(
    ...
    // key_uri is valid, but if key_name_ is not empty key_name_from_key_uri
    // must be equal to key_name_.
