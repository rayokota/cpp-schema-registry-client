...
absl::StatusOr<AsymmetricSignRequest> BuildAsymmetricSignRequest(
    ...
    absl::StatusOr<std::string> digest_string =
        ComputeDigest(data, *digest_case);
    if (!digest_string.ok()) {
...
class GcpKmsPublicKeySign : public PublicKeySign {
  public:
    ...
    GcpKmsPublicKeySign(
        ...
        google::cloud::kms::v1::PublicKey public_key,
...
absl::StatusOr<PublicKey> TryGetPublicKey(
    ...
    if (request.public_key_format() ==
        PublicKey::PUBLIC_KEY_FORMAT_UNSPECIFIED) {
        return absl::InvalidArgumentError(
            "Public Key Format must be specified");
    }
