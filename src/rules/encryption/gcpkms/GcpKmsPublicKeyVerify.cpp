...
absl::StatusOr<PublicKey> GetGcpKmsPublicKey(
    ...
    // Set format to PEM explictly for convenience, so that we use the
    // public_key response field regardless of which Cloud KMS algorithm is
    // being used.
...
CreateSignaturePublicKey(absl::string_view key_name,
    ...
    if (IsPqcAlgorithm(gcp_kms_public_key->algorithm())) {
        tink_keyset_handle = GetTinkKeySetHandleFromPqcKey(
            gcp_kms_public_key->algorithm(),
            gcp_kms_public_key->public_key().data());
    } else {
        tink_keyset_handle = GetTinkKeySetHandleFromPemKey(
            gcp_kms_public_key->algorithm(),
            gcp_kms_public_key->public_key().data());
    }
    ...
    if (signature_public_key == nullptr) {
        return absl::Status(
            absl::StatusCode::kInternal,
            absl::StrCat("Failed to cast key to "
                         "crypto::tink::SignaturePublicKey. Keyset: ",
