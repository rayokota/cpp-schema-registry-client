...
void FieldEncryptionExecutor::configure(std::shared_ptr<const ClientConfiguration> client_config,
                                          const std::unordered_map<std::string, std::string>& rule_config) {
    executor_.configure(client_config, rule_config);
}

std::string FieldEncryptionExecutor::getType() const {
    return "ENCRYPT";
}

void FieldEncryptionExecutor::close() {
    executor_.close();
}

std::unique_ptr<SerdeValue> FieldEncryptionExecutor::transformField(RuleContext& ctx, const SerdeValue& field_value) {
