...
#include "srclient/rules/encryption/LocalKmsDriver.h"
...
std::shared_ptr<crypto::tink::KmsClient> LocalKmsDriver::newKmsClient(
    ...
    if (keyUrl.find(keyUrlPrefix_) != 0) {
        throw TinkError("Key URL '" + keyUrl + "' not supported by LocalKmsDriver. " +
    ...
    try {
        ...
        throw TinkError("Failed to create LocalKmsClient: " + std::string(e.what()));
    }
}
