...
namespace std {
std::size_t hash<srclient::rest::KekId>::operator()(
    const srclient::rest::KekId &k) const {
    return std::hash<std::string>()(k.name) ^
           (std::hash<bool>()(k.deleted) << 1);
}

std::size_t hash<srclient::rest::DekId>::operator()(
    const srclient::rest::DekId &k) const {
