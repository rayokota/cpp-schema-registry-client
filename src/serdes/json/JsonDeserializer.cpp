...
nlohmann::json JsonDeserializer::deserialize(
    ...
    // Get writer schema
    auto writer_schema_raw = base_->getWriterSchema(schema_id,
                                                    subject_opt,
                                                    std::nullopt);
    auto [writer_schema, writer_schema_doc] = getParsedSchema(writer_schema_raw);
    ...
    if (latest_schema.has_value()) {
        ...
        auto [parsed_reader_schema, parsed_reader_schema_str] = getParsedSchema(reader_schema_raw);
    ...
    std::string json_string(reinterpret_cast<const char*>(message_data), message_size);
    nlohmann::json value;
    try {
