...
std::vector<uint8_t> prefixSchemaIdSerializer(
    ...
    try {
        ...
        throw SerializationError("Failed to serialize schema ID: " + std::string(e.what()));
    }
}
...
std::vector<uint8_t> headerSchemaIdSerializer(
    ...
    try {
        if (!ser_ctx.headers.has_value()) {
            throw SerializationError("Headers are required for header schema ID serialization");
        }
        ...
        switch (ser_ctx.serde_type) {
            ...
            case SerdeType::Value:
                ...
                break;
            default:
                throw SerializationError("Invalid serde type for header serialization");
        }
        ...
        throw SerializationError("Failed to serialize schema ID to header: " + std::string(e.what()));
    }
}

size_t dualSchemaIdDeserializer(
    const std::vector<uint8_t>& payload,
    const SerializationContext& ser_ctx,
    SchemaId& schema_id) {
    ...
    try {
        ...
        throw SerializationError("Failed to deserialize schema ID: " + std::string(e.what()));
    }
}

size_t prefixSchemaIdDeserializer(
    const std::vector<uint8_t>& payload,
    const SerializationContext& ser_ctx,
    SchemaId& schema_id) {
    ...
    try {
        ...
        throw SerializationError("Failed to deserialize schema ID from prefix: " + std::string(e.what()));
    }
}
